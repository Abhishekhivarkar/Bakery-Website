  ...
  try {
    ...
    for (const item of items) {
      ...
      if (item.product && mongoose.Types.ObjectId.isValid(item.product)) {
        ...
        console.log(`📦 Found database product: ${product.name}, Stock: ${product.stock}, Requested: ${item.qty}`);
    ...
    console.log(`📊 Order summary: Subtotal=${totalAmount}, Tax=${tax}, Delivery=${deliveryCharge}, Total=${grandTotal}`);

    // Create Razorpay order if payment method is razorpay
    let razorpayOrder = null;
    if (paymentMethod === "razorpay") {
      try {
        console.log(`💳 Creating Razorpay order for amount: ${grandTotal * 100} paise`);
...
exports.createSimpleOrder = async (req, res) => {
  try {
    ...
    console.log(`💰 Order total: Subtotal=₹${totalAmount}, Tax=₹${tax}, Delivery=₹${deliveryCharge}, Total=₹${grandTotal}`);

    let razorpayOrder = null;

    if (paymentMethod === "razorpay") {
      try {
        console.log(`💳 Creating Razorpay order for amount: ₹${grandTotal} (${grandTotal * 100} paise)`);
        razorpayOrder = await razorpay.orders.create({
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    const validStatuses = [
      "created", "confirmed", "preparing", "out-for-delivery", 
...
exports.getOrderStats = async (req, res) => {
  try {
    ...
    console.log(`📊 Stats: Total=${totalOrders}, Revenue=${totalRevenue[0]?.total || 0}, Today=${todayOrders}, Pending=${pendingOrders}`);
